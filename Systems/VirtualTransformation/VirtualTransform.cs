...
namespace VLib
{
    ...
    public class VirtualTransform
    {
        ...
        List<VirtualTransform> _children;
        ...
        /// <summary> Create a blank, to be used with SetData to construct a valid virtual transform. </summary>
        public VirtualTransform() {}
        
        /// <summary> Creates a new VirtualTransform. </summary>
        public VirtualTransform(Transform t, VirtualTransform parent = null) => SetData(t, parent);

        /// <summary> Allows you to create null virtual transforms a populate them later </summary>
        public void SetData(Transform t, VirtualTransform parent = null)
        {
            if (t == null)
                throw new System.ArgumentNullException(nameof(t), "Transform is NULL!");
        ...
        public int TransformID => transformID;
        ...
        public List<VirtualTransform> children => _children ??= new();
        public void AddChild(VirtualTransform child)
        {
            if (!children.AddUnique(child))
                return;
            if (child.parent != this)
                child.parent = this; // TODO: Ensure recursive call doesn't cause infinite loop or performance issues
        }

        public void RemoveChild(VirtualTransform child)
        {
        ...
        /// <summary> <inheritdoc cref="localMatrix"/> </summary>
        public ref float4x4 LocalMatrixRef => ref localMatrix;
        /// <summary> <inheritdoc cref="localMatrix"/> </summary>
        ...
        public quaternion localRotationNative
        {
        ...
        public float3 TransformDirection(float3 inDirection)
        {
        ...
        public float3 InverseTransformDirection(float3 inDirection)
        {
        ...
        public float3 TransformPoint(float3 inPoint)
        {
        ...
        public float3 InverseTransformPoint(float3 inPosition)
        {
        ...
        public void CopyTRSFrom(ref TransformAccess transformAccess)
        {
            ...
            if (any(isnan(tLocalPos)) || any(isnan(((quaternion) tLocalRot).value)) || any(isnan(tLocalScale)) || any((float3) tLocalScale == 0))
                Debug.LogError($"Nans detected in transform, ID of virtual is: {transformID}");
            
            localMatrix = float4x4.TRS(tLocalPos, tLocalRot, tLocalScale);
        }

        public void CopyTRSFrom(Transform transform)
        {
            var tLocalPos = transform.localPosition;
            var tLocalRot = transform.localRotation;
        ...
        public void CopyTRSTo(ref TransformAccess transformAccess)
        {
            // Get all at once
            localMatrix.Decompose(out float3 localPos, out quaternion localRot, out float3 localScale);
            
            if (any(isnan(localPos)) || any(isnan(localRot.value)) || any(isnan(localScale)) || any(localScale == 0))
                Debug.LogError($"Nans detected in transform, ID of virtual is: {transformID}");
            
            transformAccess.localPosition = localPos;
            transformAccess.localRotation = localRot;
            transformAccess.localScale = localScale;
        }
        
        public static implicit operator bool(VirtualTransform t) => t != null;

        #region Internal Methods
        
        float4x4 GetLocalToWorldMatrix()
        {
            float4x4 t = localMatrix; //float4x4.TRS(localPosition, m_LocalRotation, m_LocalScale);
            if (parent != null)
        ...
        void SetLocalRotation(Quaternion q) => localRotation = q;

        void SetLocalRotationSafe(Quaternion q) => SetLocalRotation(q.normalized);
