...
namespace Unity.Animation
{
    ...
    public struct AnimationCurveNativeEvaluator
    {
        ...
        /// <param name="curveNative">The curve to evaluate.</param>
        ...
        public static float Evaluate(float time, ref AnimationCurveNative curveNative, ref AnimationCurveCache cache)
        {
            ...
            if (curveNative.Type == AnimationCurveType.Hermite)
            {
                ref var leftKeyframe = ref curveNative.GetHermiteKeyframe(cache.LhsIndex);
                ref var rightKeyframe = ref curveNative.GetHermiteKeyframe(cache.RhsIndex);
            ...
            else if (curveNative.Type == AnimationCurveType.Bezier)
            {
                ref var leftKeyframe = ref curveNative.GetBezierKeyframe(cache.LhsIndex);
                ref var rightKeyframe = ref curveNative.GetBezierKeyframe(cache.RhsIndex);
            ...
            {
                curveNative.ThrowInvalidCurveType();
                return 0;
            }
        }

        static void FindSurroundingKeyframes(float time, ref UnsafeList<float> timeCurve, ref AnimationCurveCache cache,
