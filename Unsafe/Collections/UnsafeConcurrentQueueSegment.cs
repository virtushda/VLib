...
namespace System.Collections.Concurrent
{
    ...
    internal unsafe struct UnsafeConcurrentQueueSegment<T> : IDisposable
        where T : unmanaged
    {
        ...
        internal UnsafeList<Slot> _slots; // SOS's ThreadPool command depends on this name
...
#pragma warning disable 0649 // some builds don't assign to this field
        ...
        internal IntPtr _nextSegment; // SOS's ThreadPool command depends on this name // IS UnsafeConcurrentQueueSegment<T>*
        internal UnsafeConcurrentQueueSegment<T>* NextSegment => (UnsafeConcurrentQueueSegment<T>*) _nextSegment;
        internal UnsafeConcurrentQueueSegment<T>* NextSegmentVolatile => (UnsafeConcurrentQueueSegment<T>*) Volatile.Read(ref _nextSegment);
#pragma warning restore 0649
        ...
        /// </param>
        internal UnsafeConcurrentQueueSegment(int boundedLength, Allocator allocator)
        {
            // Validate the length
            /*Debug.Assert(boundedLength >= 2, $"Must be >= 2, got {boundedLength}");
            Debug.Assert(BitOperations.IsPow2(boundedLength), $"Must be a power of 2, got {boundedLength}");#1#
            if (boundedLength < 2)
                throw new ArgumentOutOfRangeException(nameof(boundedLength), "Must be >= 2");
            if (!math.ispow2((uint)boundedLength))
                throw new ArgumentOutOfRangeException(nameof(boundedLength), "Must be a power of 2");

            // Initialize the slots and the mask.  The mask is used as a way of quickly doing "% _slots.Length",
            // instead letting us do "& _slotsMask".
            _slots = new UnsafeList<Slot>(boundedLength, allocator, NativeArrayOptions.ClearMemory);
            _slots.Length = boundedLength;
            ...
            for (int i = 0; i < _slots.Length; i++)
            {
                ...
                _slots.ElementAt(i).SequenceNumber = i;
            }
            
            _frozenForEnqueues = false;
            _preservedForObservation = false;
            _headAndTail = default;
            _nextSegment = IntPtr.Zero;
        }

        public void Dispose() => _slots.DisposeSafe();

        public void DisposeRecursive()
        {
            Dispose();
            if (_nextSegment != IntPtr.Zero)
            {
                NextSegment->DisposeRecursive();
                Destroy(NextSegment, true);
                _nextSegment = IntPtr.Zero;
            }
        }

        internal static UnsafeConcurrentQueueSegment<T>* Create(int boundedLength, Allocator allocator)
        {
            UnsafeConcurrentQueueSegment<T> segment = new UnsafeConcurrentQueueSegment<T>(boundedLength, allocator);
            UnsafeConcurrentQueueSegment<T>* ptr = 
                (UnsafeConcurrentQueueSegment<T>*)UnsafeUtility.Malloc(sizeof(UnsafeConcurrentQueueSegment<T>), UnsafeUtility.AlignOf<UnsafeConcurrentQueueSegment<T>>(), allocator);
            UnsafeUtility.CopyStructureToPtr(ref segment, ptr);
            return ptr;
        }
        
        internal static void Destroy(UnsafeConcurrentQueueSegment<T>* segment, bool recursiveThroughChildren)
        {
            if (segment == null)
                return;
            if (recursiveThroughChildren && segment->_nextSegment != IntPtr.Zero)
            {
                Destroy(segment->NextSegment, recursiveThroughChildren);
                segment->_nextSegment = IntPtr.Zero;
            }
            segment->Dispose();
            UnsafeUtility.Free(segment, Allocator.Persistent);
        }
        ...
        public bool TryDequeue([MaybeNullWhen(false)] out T item)
        {
            ...
            while (true)
            {
                ...
                int sequenceNumber = Volatile.Read(ref slots.ElementAt(slotsIndex).SequenceNumber);
                ...
                if (diff == 0)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _headAndTail.Head, currentHead + 1, currentHead) == currentHead)
                    {
                        ...
                        item = slots[slotsIndex].Item.Value;
                        if (!Volatile.Read(ref _preservedForObservation))
                        {
                            ...
                            {
                                slots.ElementAt(slotsIndex).Item = default;
                            }
                            Volatile.Write(ref slots.ElementAt(slotsIndex).SequenceNumber, currentHead + slots.Length);
                        }
                        return true;
                    }
                ...
                else if (diff < 0)
                {
                    ...
                    //spinner.Spin(sleep1Threshold: -1);
                    spinner.SpinOnce();
                }
                else
                {
        ...
        public bool TryPeek([MaybeNullWhen(false)] out T result, bool resultUsed)
        {
            ...
            while (true)
            {
                ...
                if (diff == 0)
                {
                    result = resultUsed ? slots.ElementAt(slotsIndex).Item.Value : default!;
                    return true;
                }
                else if (diff < 0)
                {
            ...
            while (true)
            {
                ...
                if (diff == 0)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _headAndTail.Tail, currentTail + 1, currentTail) == currentTail)
                    {
                        ...
                        slots.ElementAt(slotsIndex).Item = item;
                        Volatile.Write(ref slots.ElementAt(slotsIndex).SequenceNumber, currentTail + 1);
                        return true;
                    }
    ...
    {
        [FieldOffset(1 * JobsUtility.CacheLineSize)] public int Head;
        [FieldOffset(2 * JobsUtility.CacheLineSize)] public int Tail;
    }
}*/