...
namespace VLib
{
    ...
    public unsafe struct VUnsafeBufferList<T> : IDisposable /*INativeDisposable*/, INativeList<T>, IEnumerable<T> // Used by collection initializers.
        where T : unmanaged
    {
        [NativeDisableUnsafePtrRestriction] public UnsafeList<T>* listData;
        /// <summary> Sorted buffer of indices that can be reused, sorting enforces uniqueness. </summary>
        [NativeDisableUnsafePtrRestriction] public UnsafeList<int>* unusedIndices;

        /// <summary> Initializes and returns a VUnsafeList with a capacity of one. </summary>
        /// <param name="allocator">The allocator to use.</param>
        public VUnsafeBufferList(Allocator allocator) : this(1, allocator)
        {
        }
        ...
        public VUnsafeBufferList(int initialCapacity, Allocator allocator, NativeArrayOptions options = NativeArrayOptions.UninitializedMemory)
        {
            this = default;
            Initialize(initialCapacity, allocator, options);
        }
        ...
        internal void Initialize(int initialCapacity, Allocator allocator, NativeArrayOptions options = NativeArrayOptions.UninitializedMemory)
        {
            ...
            listData = UnsafeList<T>.Create(initialCapacity, allocator, options);
        ...
        public bool TryAddAtIndex(int index, T value, bool allowWriteInsideCurrentListRange = true)
        {
            ...
            if (listData->Capacity <= index)
                listData->Capacity = index + 1;
            if (Length <= index)
                Length = index + 1;
        ...
        // NOTE: Removes disabled on this collection, this enforces a recycled buffer pattern.
        ...
        public void Dispose()
        {
            ...
            listData = null;
            UnsafeList<int>.Destroy(unusedIndices);
            unusedIndices = null;
        }
        ...
        /// <summary> Sets the value to a user-defined default at index and adds the index to the recycle indices list.
        /// This is the only real way to "remove" from this type of list. Indices will be sorted by default</summary>
        public void RemoveAtClear(int index, T defaultValue = default)
        {
            if (!IndexValid(index))
                return;
            // Guard the default with exclusive add
            if (unusedIndices->AddSortedExclusive(index, out _))
        ...
        public static implicit operator NativeArray<T>(VUnsafeBufferList<T> list) => list.AsArray();
