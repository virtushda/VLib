/*// TODO: FIX THIS USING THE DISABLED TEST IN THE TESTS FOLDER (TryDequeue seems not to work???) (Make the test multi-pass to await the job and allow timeout to stop test from freezing editor)
...
{
    /// <summary>
    /// Represents a thread-safe first-in, first-out collection of objects. <br/>
    /// This is NOT copy-safe!
    /// </summary>
    /// <typeparam name="T">Specifies the type of elements in the queue.</typeparam>
    /// <remarks>
    /// All public and protected members of <see cref="UnsafeConcurrentQueue{T}"/> are thread-safe and may be used
    /// concurrently from multiple threads.
    /// </remarks>
    [DebuggerDisplay("Count = {Count}")]
    public unsafe struct UnsafeConcurrentQueue<T>// : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
        where T : unmanaged
    {
        ...
        const int InitialSegmentLength = 32;
        /// <summary>
        ...
        /// </summary>
        const int MaxSegmentLength = 1024 * 1024;
        ...
        /// </summary>
        internal VUnsafeRef<int> _crossSegmentLock;
        /// <summary>The current tail segment.</summary>
        [NativeDisableUnsafePtrRestriction]
        volatile IntPtr _tail; // IS UnsafeConcurrentQueueSegment<T>*
        [NativeDisableUnsafePtrRestriction]
        /// <summary>The current head segment.</summary>
        volatile IntPtr _head; // IS UnsafeConcurrentQueueSegment<T>* // SOS's ThreadPool command depends on this name

        internal UnsafeConcurrentQueueSegment<T>* Head => (UnsafeConcurrentQueueSegment<T>*)_head;
        UnsafeConcurrentQueueSegment<T>* Tail => (UnsafeConcurrentQueueSegment<T>*)_tail;

        /// <summary>
        /// Initializes a new instance of the <see cref="UnsafeConcurrentQueue{T}"/> class.
        /// </summary>
        public UnsafeConcurrentQueue(Allocator allocator)
        {
            this.allocator = allocator;
            _crossSegmentLock = new VUnsafeRef<int>(0, allocator);
            //_tail = _head = new UnsafeConcurrentQueueSegment<T>(InitialSegmentLength);
            _tail = _head = (IntPtr)UnsafeConcurrentQueueSegment<T>.Create(InitialSegmentLength, allocator);
        }
        ...
        public ConcurrentQueue(IEnumerable<T> collection)
        {
            ...
            _tail = _head = new UnsafeConcurrentQueueSegment<T>(length);
        ...
        public void Dispose()
        {
            if (Head != null)
            {
                Head->DisposeRecursive();
                _head = IntPtr.Zero;
            }
            if (Tail != null)
            {
                Tail->DisposeRecursive();
                _tail = IntPtr.Zero;
            }
        ...
        /// elements copied from the <see cref="UnsafeConcurrentQueue{T}"/>. <paramref name="array"/> must have
        /// zero-based indexing.
        /// </param>
        ...
        /// with the SyncRoot; otherwise, false. For <see cref="UnsafeConcurrentQueue{T}"/>, this property always
        /// returns false.</value>
        bool ICollection.IsSynchronized => false; // always false, as true implies synchronization via SyncRoot#1#
        ...
        object ICollection.SyncRoot { get { ThrowHelper.ThrowNotSupportedException(ExceptionResource.ConcurrentCollection_SyncRoot_NotSupported); return default; } }#1#

        /#1#// <summary>Returns an enumerator that iterates through a collection.</summary>
        /// <returns>An <see cref="IEnumerator"/> that can be used to iterate through the collection.</returns>
        IEnumerator IEnumerable.GetEnumerator() => ((IEnumerable<T>)this).GetEnumerator();#1#
        ...
        /// <remarks>For <see cref="UnsafeConcurrentQueue{T}"/>, this operation will always add the object to the
        /// end of the <see cref="UnsafeConcurrentQueue{T}"/>
        ...
        /// <remarks>For <see cref="UnsafeConcurrentQueue{T}"/>, this operation will attempt to remove the object
        /// from the beginning of the <see cref="UnsafeConcurrentQueue{T}"/>.
        /// </remarks>
        bool IProducerConsumerCollection<T>.TryTake([MaybeNullWhen(false)] out T item) => TryDequeue(out item);#1#

        /// <summary>
        /// Gets a value that indicates whether the <see cref="UnsafeConcurrentQueue{T}"/> is empty.
        /// </summary>
        /// <value>true if the <see cref="UnsafeConcurrentQueue{T}"/> is empty; otherwise, false.</value>
        /// <remarks>
        ...
        /#1#// <summary>Copies the elements stored in the <see cref="UnsafeConcurrentQueue{T}"/> to a new array.</summary>
        /// <returns>A new array containing a snapshot of elements copied from the <see cref="UnsafeConcurrentQueue{T}"/>.</returns>
        public T[] ToArray()
        {
            // Snap the current contents for enumeration.
            SnapForObservation(out UnsafeConcurrentQueueSegment<T> head, out int headHead, out UnsafeConcurrentQueueSegment<T> tail, out int tailTail);
        ...
        /// <summary>
        /// Gets the number of elements contained in the <see cref="UnsafeConcurrentQueue{T}"/>.
        /// </summary>
        /// <value>The number of elements contained in the <see cref="UnsafeConcurrentQueue{T}"/>.</value>
        /// <remarks>
            ...
            get
            {
                ...
                while (true)
                {
                    ...
                    else
                    {
                        ...
                        using var scopeLock = _crossSegmentLock.ScopedAtomicLock();
                        ...
                        if (head == Head && tail == Tail)
                        {
                                ...
                                tailTail == Volatile.Read(ref Tail->_headAndTail.Tail))
                            {
                                ...
                                //UnityEngine.Debug.Log($"UnsafeConcurrentQueue::Count - Head count: {GetCount(Head, headHead, headTail)} + Tail count: {GetCount(Head, tailHead, tailTail)} = {count}");
                                ...
                                for (var s = Head->NextSegment; s != Tail; s = s->NextSegment)
                                {
                                    ...
                                    CheckFrozenForEnqueues(s);
                                    count += s->_headAndTail.Tail - s->FreezeOffset;
                                    //UnityEngine.Debug.Log($"UnsafeConcurrentQueue::Count - Added: {s->_headAndTail.Tail - s->FreezeOffset}");
                                }
        ...
        static int GetCount(UnsafeConcurrentQueueSegment<T>* s, int head, int tail)
        {
        ...
        /*static long GetCount(UnsafeConcurrentQueueSegment<T>* head, int headHead, UnsafeConcurrentQueueSegment<T>* tail, int tailTail)
        {
            ...
            if (head != tail)
            {
                ...
                for (UnsafeConcurrentQueueSegment<T> s = head->_nextSegment!; s != tail; s = s._nextSegment!)
                {
        ...
        /#1#// <summary>
        /// Copies the <see cref="UnsafeConcurrentQueue{T}"/> elements to an existing one-dimensional <see
        ...
        /// <see cref="UnsafeConcurrentQueue{T}"/>. The <see cref="Array">Array</see> must have zero-based
        /// indexing.</param>
        ...
        /// -or- The number of elements in the source <see cref="UnsafeConcurrentQueue{T}"/> is greater than the
        ...
        /#1#// <summary>Returns an enumerator that iterates through the <see cref="UnsafeConcurrentQueue{T}"/>.</summary>
        /// <returns>An enumerator for the contents of the <see
        /// cref="UnsafeConcurrentQueue{T}"/>.</returns>
        /// <remarks>
        ...
        /// </summary>
        void SnapForObservation(out UnsafeConcurrentQueueSegment<T> head, out int headHead, out UnsafeConcurrentQueueSegment<T> tail, out int tailTail)
        {
            lock (_crossSegmentLock) // _head and _tail may only change while the lock is held.
            {
                ...
                for (UnsafeConcurrentQueueSegment<T> s = head; ; s = s._nextSegment!)
                {
        ...
        static T GetItemWhenAvailable(UnsafeConcurrentQueueSegment<T> segment, int i)
        {
            ...
            while ((Volatile.Read(ref segment._slots.ElementAt(i).SequenceNumber) & segment._slotsMask) != expectedSequenceNumberAndMask)
            {
                spinner.SpinOnce();
            }

            // Return the value from the slot.
            return segment._slots.ElementAt(i).Item.Value;
        }

        /*private static IEnumerator<T> Enumerate(UnsafeConcurrentQueueSegment<T> head, int headHead, UnsafeConcurrentQueueSegment<T> tail, int tailTail)
        {
            Debug.Assert(head._preservedForObservation);
            Debug.Assert(head._frozenForEnqueues);
            Debug.Assert(tail._preservedForObservation);
        ...
        /// <summary>Adds an object to the end of the <see cref="UnsafeConcurrentQueue{T}"/>.</summary>
        /// <param name="item">
        /// The object to add to the end of the <see cref="UnsafeConcurrentQueue{T}"/>.
        ...
        void EnqueueSlow(T item)
        {
            while (true)
            {
                ...
                if (tail == Tail)
                {
                    ...
                    //var newTail = new UnsafeConcurrentQueueSegment<T>(nextSize);
                    var newTail = UnsafeConcurrentQueueSegment<T>.Create(nextSize, allocator);
                    var newTailIntPtr = (IntPtr)newTail;

                    // Hook up the new tail.
                    tail->_nextSegment = newTailIntPtr;
                    _tail = newTailIntPtr;
                }
            }
        }
        ...
        /// <see cref="UnsafeConcurrentQueue{T}"/> successfully; otherwise, false.
        /// </returns>
        public bool TryDequeue([MaybeNullWhen(false)] out T result)
        {
            ...
            if (head->_nextSegment == IntPtr.Zero)
            {
                result = default;
                return false;
            }
        ...
        bool TryDequeueSlow([MaybeNullWhen(false)] out T item)
        {
            while (true)
            {
                ...
                //lock (_crossSegmentLock)
                using var scopeLock = _crossSegmentLock.ScopedAtomicLock();
                var headToDispose = Head;
                if (head == headToDispose)
                {
                    _head = head->_nextSegment;
                    UnsafeConcurrentQueueSegment<T>.Destroy(headToDispose, false);
                }
            }
        }

        /// <summary>
        /// Attempts to return an object from the beginning of the <see cref="UnsafeConcurrentQueue{T}"/>
        /// without removing it.
        /// </summary>
        /// <param name="result">
        /// When this method returns, <paramref name="result"/> contains an object from
        /// the beginning of the <see cref="UnsafeConcurrentQueue{T}"/> or default(T)
        ...
        bool TryPeek([MaybeNullWhen(false)] out T result, bool resultUsed)
        {
            ...
            while (true)
            {
                ...
                var next = s->NextSegmentVolatile;
                ...
                if (next != null)
                {
                    ...
                    Debug.Assert(next == s->NextSegmentVolatile);
                    s = next;
                }
                else if (s->NextSegmentVolatile == null)
        ...
        /// <summary>
        /// Removes all objects from the <see cref="UnsafeConcurrentQueue{T}"/>.
        /// </summary>
        public void Clear()
        {
            using var scopeLock = _crossSegmentLock.ScopedAtomicLock();
        ...
        void CheckFrozenForEnqueues(UnsafeConcurrentQueueSegment<T>* segment)
        {
            if (!segment->_frozenForEnqueues)
                throw new InvalidOperationException("Segment must be frozen for enqueues.");
        }
    }

    public static class UnsafeConcurrentQueueExtensions
    {
        public static unsafe int BruteForceCount<T>(this UnsafeConcurrentQueue<T> queue)
            where T : unmanaged, IEquatable<T>
        {
            int count = 0;
            using var scopeLock = queue._crossSegmentLock.ScopedAtomicLock();
            for (var s = queue.Head; s != null; s = s->NextSegment)
            {
                for (int i = 0; i < s->_slots.Length; i++)
                {
                    if (s->_slots[i].Item.HasValue)
                        ++count;
                }
            }
            return count;
